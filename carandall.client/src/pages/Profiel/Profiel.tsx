      ...
      if (csrfToken) {
        if (data) {
          try {
            ...
            if (resultaat.ok) {
              addNotificatie("Gegevens bijgewerkt!", "success", false);
              await getHuurder();
            } else {
              addNotificatie(
                "Er is iets fout gegaan tijdens het bewerken van uw gegevens. Probeer het opnieuw!",
                "error",
                true
              );
            }
    ...
    if (csrfToken) {
      if (data) {
        try {
          ...
          if (resultaat.ok) {
            addNotificatie(
              "Verwijderingsverzoek succesvol ingediend! Over 6 maanden worden uw gegevens definitief verwijderd.",
              "success",
              false
            );
            ...
            addNotificatie(
              "Er is iets fout gegaan tijdens het aanmaken van het verwijderingsverzoek. Probeer het opnieuw!",
              "error",
              true
            );
          }
